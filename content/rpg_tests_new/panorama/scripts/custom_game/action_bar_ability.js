...
function UpdateAbility()
{
	...
	var canBeUpged = lvlup && Abilities.GetHeroLevelRequiredToUpgrade(ability) <= Entities.GetLevel(queryUnit);
	var noLevel =( 0 == Abilities.GetLevel(ability) );
	var isCastable = (!Abilities.IsPassive(ability) && !noLevel) || canBeUpged;
	...
	$.GetContextPanel().SetHasClass( "no_level", noLevel && !canBeUpged	);
	...
	$( "#AbilityImage" ).contextEntityIndex = Players.GetLocalPlayerPortraitUnit();
	...
	else
	{
		...
		$( "#CooldownOverlay" ).style.width = ((cooldownPercent+7)*(100/103))+"%";
	}
	
	//$.Schedule(0.03, UpdateAbility );
}
...
{ 
	if(Game.IsInAbilityLearnMode()){
		$('#compiling').visible = true;
		$('#BorderOverlay').visible = false;
		lvlup = true;
	}else{
		$('#compiling').visible = false;
		$('#BorderOverlay').visible = true;
		lvlup = false;
	}
	//$.Msg('HI');
	UpdateAbility();
	$.Schedule(0.03, EnableLevelup);
}
EnableLevelup();
$('#compiling').visible = false;
(function()
{
	...
	CustomNetTables.SubscribeNetTableListener("ui_enable_lvlup", EnableLevelup);
	RebuildAbilityUI();
